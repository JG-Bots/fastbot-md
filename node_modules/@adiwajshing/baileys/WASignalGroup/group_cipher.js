...
class GroupCipher {
  ...
  async decrypt(senderKeyMessageBytes) {
    const record = await this.senderKeyStore.loadSenderKey(this.senderKeyName);
    if (!record) throw new Error(`Nenhuma chave de remetente para: ${this.senderKeyName}`);
  ...
  getSenderKey(senderKeyState, iteration) {
    ...
    if (senderChainKey.getIteration() > iteration) {
      ...
      throw new Error(
        `Mensagem recebida com contador antigo: ${senderChainKey.getIteration()}, ${iteration}`
      );
    }

    if (senderChainKey.getIteration() - iteration > 2000) {
      throw new Error('Mais de 2000 mensagens no futuro!');
    }
